...
def load_model(log_root, model_nm, dataset_nm, cl, ml, sub=2):
    ...
    weight_path = '/'.join([log_root, 'lno1d', dataset_nm, weight_nm])
...
def inference(idx, model, dataset, normalizer, device):
    ...
    with torch.no_grad():
        ...
        test_l2 = rl2(u_.view(bsz, -1), u.view(bsz, -1)).item()
