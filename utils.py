...
def ml_matrix_vector_multiplication(K, K_lst, u, nl, nbrs, h):
    lb_lst = {}
    rb_lst = {}

    u_band_lst = {}
    for l in range(nl-1):
        if nl-l-1 in nbrs.keys():
            w_lb = h * torch.einsum('bcmn, bcn-> bcm', K[:,:,[0]], u)
            w_rb = h * torch.einsum('bcmn, bcn-> bcm', K[:,:,[0]], u)        
            lb_lst[nl-l-1] = w_lb
            rb_lst[nl-l-1] = w_rb

            u_band = u[:,:,nbrs[nl-l-1][1]] * nbrs[nl-l-1][2].to(u)
            u_band_even = u_band[:,:,::2]
            u_band_odd = u_band[:,:,1::2]
            u_band_lst[nl-l-1] = [u_band_even[:,:,:,::2], u_band_odd]
        
        K = injection1d_rows(K) 
        K = injection1d_cols(K)
        u = restrict1d(u)
        h = 2*h
    ...
    for l in range(0,nl):
        if l == 0:
            w = h * torch.einsum('bcmn, bcn-> bcm', K[l], u)
        else:
            h = h/2
            # even correction
            if l in nbrs.keys():
                w += (K[l][0] * u_band_lst[l][0]).sum(axis=-1)*h
            
            w = interp1d(w)

            # odd correction
            if l in nbrs.keys():
                w[:,:,1::2] = w[:,:,1::2] + (K[l][1] * u_band_lst[l][1]).sum(axis=-1)*h
    return w
...
def init_records(log_root, args):
    resolution = '8193' if args.fine else '513'
    task_nm = '_'.join([args.task, resolution])

    if args.model == 'toep_mg':
        exp_nm = '-'.join([args.model, str(args.k), str(args.m), str(args.seed)])
    
    if args.model == 'dd_mg':
        exp_nm = '-'.join([args.model, str(args.k), str(args.m), str(args.seed)])    

    if args.model in ['lr_mg', 'mlplr_mg']:
        exp_nm = '-'.join([args.model, str(args.nl), str(args.nc), str(args.m), str(args.k), str(args.r), str(args.seed)])    
        
    if args.model in ['toep_gl', 'gl']:
        exp_nm = '-'.join([args.model, args.act, str(args.seed)])    
    
    if args.model == 'lr_gl':
        exp_nm = '-'.join([args.model, args.act, str(args.r), str(args.seed)])    
...
def train_model(train_loader, model, opt, device, loss_cfg):
    model.train()
    train_rl2 = 0
    ...
    for u, w in train_loader:
        ...
        if isinstance(opt, torch.optim.Adam):
            ...
            if loss_cfg['rl2'] is not None:
                # calc loss
                loss_rl2 = rl2_error(w_, w)
            else:
                loss_rl2 = 0
            
            if loss_cfg['gnorm'] is not None:
                loss_grad = gradient_loss(
                    k, h, loss_cfg['gnorm']['thr'], loss_cfg['gnorm']['w'])
            else:
                loss_grad = 0

            loss = loss_rl2 + loss_grad



            # # logarithm 
            # # loss_grad = gradient_loss(k, h, max_grad=0.5, s=2.)

            # # laplace
            # loss_grad = gradient_loss(k, h, max_grad=1e-5, w='one')
            # loss = loss + loss_grad
...
def eval_model(test_loader, model, device):
    ...
    if model.mtype == 'lr_gl':
        k_ = torch.einsum('b c n, b c m -> b n m', k_[0], k_[1])
    elif model.mtype in ['lr_mg', 'dd_mg']:
        k_ = k_[0]

    return test_rl2, k_.detach().cpu().numpy()
