...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="fourier neural operator 1d profile")
    args = get_arguments(parser)
    cfg_root = '/'.join(current_path.split('/')[:-1] + ['cfgs'])
    with open(os.path.join(cfg_root, f'ft1d-cfg.yaml')) as f:
    ...
    model_nm = f'ft1d-h{model_cfg.nhead}-w{model_cfg.width}'+ \
               f'-{tra_res}-{test_res}'+ \
               f'-cl{args.clevel}-ml{mlevel}'
    
    log_root = os.path.join(data_cfg.plog_dir, gpu_nm, f'exp1d/ft1d/')
